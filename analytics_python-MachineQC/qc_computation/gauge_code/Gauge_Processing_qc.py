...
def Vibration_Quality_Control(ac_stat_list,subassembly_instance,subassembly,current_datetime_stamp,log_json):
    try:
        ...
        add_log_info(log_json,current_datetime_stamp,"Vibration_Quality_Control","error",error_text)
...
def Vacuum_Quality_Control(vac_stat_list,subassembly_instance,subassembly,gauge_rule_list=None,current_datetime_stamp=0,log_json=None):                     #intermediate_stored_objects, , subassembly_instance, gauge_rule_list):
    try:
        ...
        if output_conditions['combined_input_exception_indicator']==1:
            output={"color":["red"], "value":[-1], "trendExistence":False, "success":"true", "purpose":"qc", "reason":"High Vac calculation not possible due to insufficient data"}
...
#=================need to write catch========================
        ...
        error_text=traceback.format_exc()
        add_log_info(log_json,current_datetime_stamp,"Vacuum_Quality_Control","error",error_text)
...
def QC_Gauge_Output( subassembly_instance,Vibration_QC_output,Vacuum_QC_output,ac_stat_list,vac_stat_list,current_datetime_stamp,log_json):
    try:
        ...
        if ac_stat_list is not None:
            ...
            final_output[gauge_name_list] =  deepcopy(Vibration_QC_output)  
    
        if vac_stat_list is not None:
            gauge_name_list = subassembly_instance +':  ' + 'Vacuum QC'
            final_output[gauge_name_list] = deepcopy(Vacuum_QC_output)
            

        return final_output
    except Exception:
        error_text=traceback.format_exc()
        add_log_info(log_json,current_datetime_stamp,"QC_Gauge_Output","error",error_text)
        return {}
...
# =============================================================================
#         if current_qc_state_count%2==0 and previous_qc_state_count%2==1:
#             if "qc_complete" not in stored_object or stored_object["qc_complete"]:
#                 stored_object["qc_complete"]=False
# =============================================================================
    else:
    ...
    if reset_gauge_indicator == "Yes":
        ...
        if qc_external_gauge_name in qc_gauge_output:
        ...
        if manufacturer in manufacturer_list:
            ...
            if ac_stat_list is not None or xl_stat_list is not None:
                ...
                if vibration_QC_gauge_progress_percent<100:  
                    ...
                    Vibration_QC_output = Vibration_Quality_Control(stat_list,subassembly_instance,subassembly,current_datetime_stamp,log_json)
                    #==================need to discuss============================
                    ...
                    if  stored_object['vibration_qc']['qc_gauge_progress_percent']==100:
                        ...
                        if stored_object['vibration_qc']['qc_gauge_red_percent'] >= 80:
                            ...
                            #logger.log('Vibration Machine QC process completed => Not Ok', 'success', False, None)
                        else: 
                            Vibration_QC_output['color']=['green']   
                            #logger.log('Vibration Machine QC process completed => Ok', 'success', False, None)
...
#                         print(Vibration_QC_output)
            ...
            if vac_stat_list is not None:
                ...
                if vacuum_QC_gauge_progress_percent<100:    
                    Vacuum_QC_output=Vacuum_Quality_Control(vac_stat_list,subassembly_instance,subassembly,gauge_rule_list,current_datetime_stamp,log_json)
                    #==================need to discuss============================
                    ...
                    if  stored_object['vacuum_qc']['qc_gauge_progress_percent']==100:
                        ...
                        if stored_object['vacuum_qc']['qc_gauge_red_percent'] >= 80:
                            ...
                            #logger.log('Vacuum Machine QC process completed => Not Ok', 'success', False , None)
                        else: 
                            Vacuum_QC_output['color']=['green']   
                            #logger.log('Vacuum Machine QC process completed => Ok', 'success', False, None)
...
#                         print(Vacuum_QC_output)
            ...
            qc_gauge_output = QC_Gauge_Output(subassembly_instance,Vibration_QC_output,Vacuum_QC_output,stat_list,vac_stat_list,current_datetime_stamp,log_json)
            ...
            #... Individual QC Process: Vibration ...#
            
            if 0 < stored_object['vibration_qc']['qc_gauge_progress_percent'] < 100:
                
                if Vibration_QC_output != {}:
                    
                    try:
                        if Vibration_QC_output["color"]==["green"]:
                            logger.log('Vibration Machine QC process running : [Green] ' + str(stored_object['vibration_qc']['qc_gauge_green_percent'])+ " %", 'success', False, None)
                        elif Vibration_QC_output["color"]==["red"]:
                            logger.log('Vibration Machine QC process running : [Red] ' + str(stored_object['vibration_qc']['qc_gauge_red_percent'])+ " %", 'failed', False, None)
                            
                    except Exception:
                        error_text=traceback.format_exc()
                        add_log_info(log_json,current_datetime_stamp,"Gauge_Computation","error",error_text+"\n\nVibration_QC_output: {}".format(Vibration_QC_output))
                        
            elif stored_object['vibration_qc']['qc_gauge_progress_percent'] == 100:
                if Vibration_QC_output != {}:
                    try:
                        if Vibration_QC_output["color"]==["green"]:
                            logger.log('Vibration Machine QC process completed => Ok', 'success', False, None)
...
# =============================================================================
#             else:
#                 logger.log('Vibration Machine QC process initialized', 'success', False, None)
# =============================================================================

            #... Individual QC Process: Vacuum ...#  
            
            if 0 < stored_object['vacuum_qc']['qc_gauge_progress_percent'] < 100:
                
                if Vacuum_QC_output != {}:
                    try:
                        if Vacuum_QC_output["color"]==["green"]:
                            logger.log('Vacuum Machine QC process running : [Green] ' + str(stored_object['vacuum_qc']['qc_gauge_green_percent'])+ " %", 'success', False, None)
                        elif Vacuum_QC_output["color"]==["red"]:
                            logger.log('Vacuum Machine QC process running : [Red] ' + str(stored_object['vacuum_qc']['qc_gauge_red_percent'])+ " %", 'failed', False, None)
                            
                    except Exception:
                        error_text=traceback.format_exc()
                        add_log_info(log_json,current_datetime_stamp,"Gauge_Computation","error",error_text+"\n\nVacuum_QC_output: {}".format(Vacuum_QC_output))

            
            elif stored_object['vacuum_qc']['qc_gauge_progress_percent'] == 100:
...
# =============================================================================
#             else:
#                 logger.log('Vacuum Machine QC process initialized', 'success', False, None)
# =============================================================================
                    
            #... Overall QC Message To Be Published in UI ...#
            

            #.. If Both QC Processes Are Complete ...#
# =============================================================================
...
#                 
#                 if "qc_complete" not in stored_object or not stored_object["qc_complete"]:
#                     print("QC Complete")
#                     logger.update_qc_input_configuration(False)
#                     stored_object["qc_complete"]=True
#                     
#                     
#                     
#                 
